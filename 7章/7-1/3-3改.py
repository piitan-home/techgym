...
class Main():
    ...
    def _show_team(self):
        txt = 'チーム一覧\n'
        for i, team in enumerate(self._team):
            txt += f'{team.name}:{i+1}\n'
        print(txt.rstrip())

    def _get_team(self, text=''):
        while True:
            team = input(text)
            if team.isdecimal():
                team = int(team)
                if 0 < team <= len(self._team):
                    return team-1
    ...
    def main(self):
        ...
        self._create_teams()
        self._show_team()
        self._one_game(self._team[self._get_team('自分のチームを選択してください:')],
                       self._team[self._get_team('相手のチームを選択してください:')])
